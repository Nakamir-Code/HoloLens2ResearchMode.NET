...
namespace winrt::HoloLens2ResearchMode::implementation
{
	...
	// Helper function to handle SEH for GetNextBuffer
	static HRESULT SafeGetNextBuffer(::IResearchModeSensor* pSensor, ::IResearchModeSensorFrame** pSensorFrame)
	{
		HRESULT hr;
		__try
		{
	...
	winrt::HoloLens2ResearchMode::IResearchModeSensorFrame ResearchModeCameraSensor::GetNextBuffer()
	{
		...
		HRESULT hr = SafeGetNextBuffer(m_pSensor.get(), &pSensorFrame);
		winrt::check_hresult(hr);
